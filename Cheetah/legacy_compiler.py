...
class MethodCompiler(object):
    ...
    def _addAutoSetupCode(self):
        ...
        self.indent()
        self.addChunk('if self.avg_length and "_cheetah" in str(CheetahStringIO):')
        self.indent()
    ...
    def _addAutoCleanupCode(self):
        ...
        if not self._isGenerator:
            ...
            self.addChunk('if type(self).avg_length is None:')
            self.indent()
            self.addChunk('type(self).avg_length = len(result)')
            self.dedent()
            self.addChunk('type(self).avg_length = (type(self).avg_length + len(result)) // 2')
